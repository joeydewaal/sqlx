...
impl PgConnection {
    ...
    /// Starts a temporary pipe to the background worker, the worker sends responses back until
    /// the the condition of `PipeUntil` is met.
    pub(crate) async fn start_pipe_async<'c, F, Fut>(
        &'c self,
        callback: F,
    ) -> sqlx_core::Result<ConnManager<'c>>
    where
        F: FnOnce(&mut MessageBuf) -> Fut,
        Fut: Future<Output = sqlx_core::Result<PipeUntil>>,
    {
        let mut buffer = MessageBuf::new();
        let wait_type = (callback)(&mut buffer).await?;
        let (request, receiver) = buffer.finish(wait_type);
        self.inner
            .chan
            .unbounded_send(request)

...
use sqlx_core::{query_as::query_as, Error};

use crate::{
    connection::describe::{TypCategory, TypType},
    type_info::{PgCustomType, PgType},
    types::Oid,
    PgConnection, PgTypeInfo, PgTypeKind,
};
...
ORDER BY
    oid
        ...
        Self::recursive_find(&mut recursive_values, oid).ok_or(Error::RowNotFound)
    }

    fn recursive_find(fetched: &mut Vec<FetchByOid>, oid: Oid) -> Option<PgTypeInfo> {
        println!("recursive {oid:?}");
        if let Some(_built_in) = PgTypeInfo::try_from_oid(oid) {
            return Some(_built_in);
        }

        let found_row = fetched.swap_remove(fetched.iter().position(|r| r.fetched_oid == oid)?);
        ...
        match (typ_type, category) {
            (Ok(TypType::Domain), _) => {
                let domain_type = Self::recursive_find(fetched, found_row.base_type)?;
            ...
            (Ok(TypType::Composite), Ok(TypCategory::Composite)) => {
                ...
                for (name, col_oid) in found_row.composite_fields {
                    fields.push((name, Self::recursive_find(fetched, col_oid)?));
                }
